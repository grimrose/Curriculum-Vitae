# 職務経歴書

## 基本情報

| key         | value                                                               |
| ----------- | ------------------------------------------------------------------- |
| Name        | よしだ(Yoshida)                                                     |
| GitHub      | [grimrose](https://github.com/grimrose)                             |
| Blog        | [open build/reports/life/index.html](http://www.grimrose.org/blog/) |
| Twitter     | [@grimrose](https://twitter.com/grimrose)                           |
| Speakerdeck | [grimrose](https://speakerdeck.com/grimrose)                        |

## スキル

### テクニカルスキル

| 項目                         | 名称                | 備考                                                  |
| ---------------------------- | ------------------- | ----------------------------------------------------- |
| OS                           | Linux(CentOS, RHEL) | 手順書もとにインストールが可能。                      |
|                              | Windows             | 手順書もとにインストールが可能。                      |
| 言語                         | Java                | 1999 年~, Java7 でのプログラミングが可能。            |
|                              | JavaScript          | 1999 年~                                              |
|                              | Scala               | 2015 年~                                              |
|                              | PHP                 | 2015 年~                                              |
|                              | TypeScript          | 2015 年~                                              |
|                              | Groovy              | 趣味で約 4 年                                         |
|                              | Python              | 簡単なプログラミングが可能。                          |
| フレームワーク or ライブラリ | Skinny Framework    | 2015 年~                                              |
|                              | ScalikeJDBC         | 2015 年~                                              |
|                              | Akka                | 2017 年~                                              |
|                              | Akka Streams        | 2017 年~                                              |
|                              | Selenium            | Geb であればプログラミングが可能。                    |
|                              | Node.js             | 簡単なプログラミングが可能。                          |
|                              | jQuery              | 簡単なプログラミングが可能。                          |
|                              | Angular             | 簡単なプログラミングが可能。                          |
|                              | Laravel             | 2016 年~                                              |
|                              | FuelPHP             | 2015 年~                                              |
| DB                           | MySQL               | 環境構築から運用保守まで可能。                        |
|                              | PostgreSQL          | 環境構築から運用保守まで可能。                        |
|                              | Oracle              | 手順書をもとにインストールが可能。PL/SQL の経験あり。 |
| その他                       | Gradle              | plugin の作成が可能。                                 |
|                              | Ansible             | 手順書からの移行が可能。                              |
|                              | Docker              | 環境構築から運用保守まで可能。                        |
|                              | Jenkins             | 環境構築から運用保守まで可能。                        |
|                              | embulk              | 環境構築から運用保守まで可能。                        |
|                              | digdag              | 環境構築から運用保守まで可能。                        |

### その他

- Yokohama.groovy の 2 代目主宰

  - [connpass](https://connpass.com/series/253/)
  - [GitHub](https://github.com/yokohamagroovy)

- ScalaMatsuri スタッフ

  - [2016](http://2016.scalamatsuri.org/ja/staff/)
  - [2017](http://2017.scalamatsuri.org/ja/staff/)
  - [2018](http://2018.scalamatsuri.org/ja/staff/)

- TDDBC のアシスタント
  - Java
  - Groovy
  - JavaScript
  - Scala

## 強み

- 勤怠、給与、人事に関する業務知識
- Scala による Web アプリケーションの開発
- PostgreSQL・MySQL の保守運用
- テスト駆動開発に関する知識及び指導経験
- 新しい技術を学び続ける向上心
- データの収集から可視化までの  要件定義から保守運用までの

## やったことはないが興味があるもの

- Kubernetes の導入
- In-memory Data Grid を活用した分散処理
- Akka Cluster を利用した業務システム

## 登壇歴

| Date       | Event                                  | Slide                                                                                                                                                                                                                            |
| ---------- | -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2018-02-09 | 吉祥寺.pm13                            | [イミュータブルデータ・モデルを 1 年使ってみた](https://nbviewer.jupyter.org/format/slides/github/grimrose/kichijojipm-13/blob/master/LT.ipynb#/)                                                                                |
| 2018-02-03 | BuriKaigi2018                          | [とあるマーケティング部隊とエンジニアと Scala の導入](https://gist.github.com/grimrose/aacbad883617b025f3300d220e266217)                                                                                                         |
| 2017-09-08 | Scala 関西 Summit2017                  | [Akka Streams へ移行しとるんやけどちょっと聞いてや](https://nbviewer.jupyter.org/format/slides/github/grimrose/scala-kansai-summit-2017/blob/master/Migration%20to%20Akka%20Streams.ipynb#/)                                     |
| 2017-06-07 | Workflow Engines Night                 | [とあるマーケティング部隊での digdag の活用事例](https://gist.github.com/grimrose/5bea98db4c82056dad1ab84d6653308e)                                                                                                              |
| 2017-05-16 | Embulk Meetup Tokyo #3                 | [とあるマーケティング部隊での embulk の活用事例](https://gist.github.com/grimrose/1377fce36840b6ef7536d22dc63e7761)                                                                                                              |
| 2016-05-21 | JJUG CCC 2016 Spring                   | [十徳ナイフとしての Gradle](https://nbviewer.jupyter.org/format/slides/github/grimrose/JJUG-CCC-2016-Spring/blob/master/Gradle%20as%20Army%20Knife.ipynb#/)                                                                      |
| 2016-02-13 | 第十四回 #渋谷 java                    | [scalaxb と Skinny Framework で SOAP に立ち向かう](http://nbviewer.jupyter.org/format/slides/github/grimrose/shibuya-java-14/blob/master/scalaxb%20and%20skinny%20framework.ipynb#/)                                             |
| 2015-12-05 | 合同勉強会 in 大都会岡山 -2015 Winter- | [業務アプリケーションを Electron で作ってみた](http://nbviewer.jupyter.org/format/slides/github/grimrose/gbdaitokai2015/blob/4b8f0eefd0ee5cdcca1e0af072c6754b2b9b3163/Business%20applications%20written%20in%20Electron.ipynb#/) |

## 職務経歴

### 2014/12 - : 人材紹介会社

職務: Web アプリケーションエンジニア、データエンジニア

#### マーケティング部門データ分析チーム

- インフラ設計
- サービスアーキテクチャ検討
- 技術検証・選定
- 要件定義
- 基本設計
- 詳細設計
- 実装
- テスト
- 開発環境改善
- 保守、運用、障害対応
- 基幹システムとの連携
- 業務改善

### 2007/4 - 2014/11: システム開発会社

#### 開発経歴

|     |   年 |  月 | 業務内容             | 開発環境                                                     | 参加形態               | 担当         |
| --: | ---: | --: | :------------------- | :----------------------------------------------------------- | :--------------------- | :----------- |
|     | 2014 |   7 | A 社給与管理システム | Windows, Linux, Java, JavaScript, Tomcat, PostgreSQL         | 分析                   | PG           |
|   - | 2014 |  11 |                      |                                                              |                        |
|     | 2014 |   5 | W 社勤怠管理システム | Windows, Linux, Java, JavaScript, Glassfish, PostgreSQL      | 分析, 設計, 製造       | リーダー, PG |
|   - | 2014 |   6 |                      |                                                              |                        |
|     | 2013 |   8 | 自社給与管理システム | Windows, Linux, Java, JavaScript, Tomcat, PostgreSQL         | 分析, 製造             | PG           |
|   - | 2014 |   2 |                      |                                                              |                        |
|     | 2007 |   6 | 自社勤怠管理システム | Windows, Linux, Java, JavaScript, Tomcat, MySQL, PostgreSQL  | 分析, 設計, 製造, 運用 | PG           |
|   - | 2014 |   6 |                      |                                                              |                        |
|     | 2011 |  11 | 自社給与管理システム | Windows, Linux, Java, JavaScript, Tomcat, PostgreSQL         | 製造                   | PG           |
|   - | 2012 |  11 |                      |                                                              |                        |
|     | 2011 |   8 | A 社給与管理システム | Windows, Linux, Java, JavaScript, Tomcat, PostgreSQL         | 製造                   | PG           |
|   - | 2011 |  11 |                      |                                                              |                        |
|     | 2010 |  12 | H 社勤怠管理システム | Windows, Java, JavaScript, Tomcat, MySQL                     | 分析, 設計, 製造, 運用 | PG           |
|   - | 2011 |   7 |                      |                                                              |                        |
|     | 2010 |   7 | C 社勤怠管理システム | Windows, Biz/Browser, PL/SQL, Oracle                         | 分析, 設計, 製造       | PG           |
|   - | 2010 |  11 |                      |                                                              |                        |
|     | 2010 |   5 | A 社勤怠管理システム | Windows, Linux, Java, JavaScript, Tomcat, PostgreSQL, Oracle | 分析, 設計             | PG           |
|   - | 2010 |   6 |                      |                                                              |                        |
|     | 2008 |   9 | F 社勤怠管理システム | Windows, Linux, Java, JavaScript, Tomcat, MySQL              | 分析, 設計, 製造, 運用 | リーダー, PG |
|   - | 2014 |   - |                      |                                                              |                        |
|     | 2007 |   8 | V 社人材派遣システム | Windows, Linux, Java, JavaScript, Tomcat, MySQL              | 分析, 設計, 製造       | PG           |
|   - | 2008 |   1 |                      |                                                              |                        |

#### 詳細

| 業務内容             | 備考                                                                                                                     |
| :------------------- | :----------------------------------------------------------------------------------------------------------------------- |
| W 社勤怠管理システム | W 社のシステムと連携する為に、自社の勤怠管理システムを JAX-RS と適合させることが出来ました。                             |
| 自社勤怠管理システム | バージョンアップの際にフレームワークの要件定義にも携わりました。                                                         |
| 自社給与管理システム | A 社での案件の経験を活かして自社のパッケージをバージョンアップすることが、出来ました。                                   |
| A 社給与管理システム | 後半からの参加でしたが、積極的に改善を行うことで品質向上に貢献することが、出来ました。                                   |
| H 社勤怠管理システム | 保守性や開発チームの特性を考慮して Seasar2 をベースに開発する提案を行い、採用されました。                                |
| C 社勤怠管理システム | 新しい言語であっても、これまでの勤怠管理の経験を活かして開発することが出来ました。                                       |
| A 社勤怠管理システム | 既存のグループウェアと連携する機能と、新しくバージョンの勤怠管理システムに必要な機能の調査分析と設計を行いました。       |
| F 社勤怠管理システム | 新規導入から保守まで全行程に携わりました。また、他社のシステムと連携するプロジェクトにも携わりました。                   |
| V 社人材派遣システム | 参画する前に人事の業務知識を勤怠に関する箇所から学び始めました。また、ユーザが使いやすい UI を考える難しさを知りました。 |
